        ...
        <el-table :data="list">
          ...
          <el-table-column label="操作" width="280px">
            <template v-slot="{ row }">
              ...
              <el-popconfirm title="确定删除这条内容吗?" @onConfirm="confirmDel(row.id)">
...
export default {
  ...
  data() {
    return {
      ...
      total: 0, // 记录员工的总数
      list: [], // 存储员工列表数据
      showExcelDialog: false, // 控制excel的单层显示和隐藏
      showRoleDialog: false, // 控制角色弹层
      roleList: [], // 接收角色列表
      roleIds: [], // 用于双向绑定数据
      currentUserId: null // 用于记录当前点击的用户ID
    }
  },
  created() {
    this.getDepartment()
  ...
  methods: {
    ...
    changeValue() {
      ...
      clearTimeout(this.timer) // 清理上次的定时器
    ...
    async exportEmployee() {
      const result = await exportEmployee() // 导出所以的员工接口
    ...
    async confirmDel(id) {
      ...
      this.getEmployeeList()
      this.$message.success('员工成功删除')
    },
    async btnRole(id) {
      this.roleList = await getEnableRoleList()
      // 记录当前点击的ID,后续操作中要存取给对应的用户
